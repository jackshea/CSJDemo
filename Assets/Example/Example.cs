...
public sealed class Example : MonoBehaviour
{
    ...
    private int mSplashExpressWidthDp = Screen.width;
    private int mSplashExpressHeightDp = Screen.height;
    ...
    public void LoadSplashAd()
    {
        ...
        var adSlot = new AdSlot.Builder()
            ...
            .SetExpressViewAcceptedSize(mSplashExpressWidthDp, mSplashExpressHeightDp)  //普通开屏也需要设置模版size  
#endif
    ...
    private void InitScreenSize()
    {
        ...
        float scale = unityContext.Call<AndroidJavaObject>("getResources").Call<AndroidJavaObject>("getDisplayMetrics").Get<float>("density");
...
#endif
            .SetImageAcceptedSize(Screen.width, Screen.height)
            .SetExpressViewAcceptedSize(mSplashExpressWidthDp, mSplashExpressHeightDp)
            .Build();
        Debug.LogError("UnityEngine.Screen.width " + UnityEngine.Screen.width + "  UnityEngine.Screen.height " + UnityEngine.Screen.height);
#if UNITY_IOS
        this.splashAd = (this.AdNative.LoadSplashAd_iOS(adSlot, new SplashAdListener(this), callbackOnMainThread));
#else
        this.AdNative.LoadSplashAd(adSlot, new SplashAdListener(this, GetActivity(), GetSplashAdManager()), SPLASH_TIME_OUT, callbackOnMainThread);
    ...
    public void LoadNativeBannerAd()
    {
        ...
        var adSlot = new AdSlot.Builder()
            ...
            .SetAdCount(1)
            .Build();
        this.AdNative.LoadNativeAd(adSlot, new NativeAdListener(this, AdSlotType.Banner), callbackOnMainThread);
    }
    ...
    public void LoadNativeIntersititialAd()
    {
        ...
        var adSlot = new AdSlot.Builder()
            ...
            .SetAdCount(1)
            .Build();
        this.AdNative.LoadNativeAd(adSlot, new NativeAdListener(this, AdSlotType.InteractionAd), callbackOnMainThread);
    }
...
#else
        bannerAd.RenderNative(GetActivity(),new NativeAdDislikeCallback(this),AdSlotType.Banner);
#endif
    }
...
#else
        bannerAd.RenderNative(GetActivity(), new NativeAdDislikeCallback(this), AdSlotType.InteractionAd);
#endif
    }
    ...
    private class NativeAdDislikeCallback :IDislikeInteractionListener
    {
        private Example example;

        public NativeAdDislikeCallback(Example example)
        {
            this.example = example;
        }
        ...
        public void OnCancel()
        {
            Debug.LogError("native ad dislike OnCancel");
            if (Thread.CurrentThread.ManagedThreadId == mainThreadId)
            {
                this.example.information.text = "native ad dislike OnCancel";
            }
        }

        public void OnShow()
        {
            Debug.LogError("native ad dislike OnShow");
            if (Thread.CurrentThread.ManagedThreadId == mainThreadId)
            {
                this.example.information.text = "native ad OnShow:";
            }
        }

        public void OnSelected(int var1, string var2, bool enforce)
        {
            Debug.LogError("native ad dislike OnSelected:" + var2);
            if (Thread.CurrentThread.ManagedThreadId == mainThreadId)
            {
                this.example.information.text = "native ad OnSelected:";
            }
        }
    }
    private sealed class NativeAdInteractionListener : IInteractionAdInteractionListener
    {
        ...
        {
            Debug.Log($"NativeAd creative click   on main thread: {Thread.CurrentThread.ManagedThreadId == mainThreadId}");
            if (Thread.CurrentThread.ManagedThreadId == mainThreadId)
            {
        ...
        public void OnAdDismiss()
        {
            ...
            example.intersititialAd?.Dispose();
        }
    ...
    {
        private Example example;
        private AdSlotType type;//0->banner,1->interstitial
        public NativeAdListener(Example example, AdSlotType type)
        {
            this.example = example;
            this.type = type;
        }
        ...
        public void OnNativeAdLoad(NativeAd[] ads)
        {
            ...
            {
                Debug.Log($"OnNativeAdLoad ads array is null on main thread: {Thread.CurrentThread.ManagedThreadId == mainThreadId}");
                if (Thread.CurrentThread.ManagedThreadId == mainThreadId)
                {
                    this.example.information.text = "OnNativeAdLoad ads array is null ";
                }
    ...
    private sealed class RewardAdInteractionListener : IRewardAdInteractionListener
    {
        ...
        {
            var logString = "OnRewardArrived verify:" + isRewardValid + " rewardType:" + rewardType + " extraInfo:" + extraInfo +
                            $" on main thread: {Thread.CurrentThread.ManagedThreadId == mainThreadId}";
            Debug.Log(logString);
            if (Thread.CurrentThread.ManagedThreadId == mainThreadId)
            {
    ...
    private sealed class RewardAgainAdInteractionListener : IRewardAdInteractionListener
    {
        ...
        public void OnAdVideoBarClick()
        {
            ...
            {
                this.example.information.text =
                    "Callback --> 第 " + mNowPlayAgainCount + " 次再看 rewardPlayAgain bar click";
            }
        }
        ...
        public void OnVideoSkip()
        {
            ...
            if (Thread.CurrentThread.ManagedThreadId == mainThreadId)
            {
                this.example.information.text = "Callback --> 第 " + mNowPlayAgainCount + " 次再看 rewardPlayAgain has OnVideoSkip";
            }
        }
        ...
        public void OnVideoComplete()
        {
            ...
            if (Thread.CurrentThread.ManagedThreadId == mainThreadId)
            {
                this.example.information.text = "Callback --> 第 " + mNowPlayAgainCount + " 次再看 rewardPlayAgain complete";
            }
        }
        ...
        public void OnVideoError()
        {
            ...
            if (Thread.CurrentThread.ManagedThreadId == mainThreadId)
            {
                this.example.information.text = "Callback --> 第 " + mNowPlayAgainCount + " 次再看 rewardPlayAgain error";
            }
        }
            ...
            bool rewardVerify, int rewardAmount, string rewardName, int rewardType, float rewardPropose)
        {
            ...
            if (Thread.CurrentThread.ManagedThreadId == mainThreadId)
            {
                this.example.information.text =
                    ...
                    " name:" + rewardName + " rewardType:" + rewardType + " rewardPropose:" + rewardPropose + " nowPlayAgainCount: " + mNowPlayAgainCount;  
            }
        }
    ...
    private sealed class SplashAdListener : ISplashAdListener
    {
        ...
        {
            Debug.Log("splash load OnSplashLoadFail:" + code + ":" + message + $" on main thread: {Thread.CurrentThread.ManagedThreadId == mainThreadId}");
            if(Thread.CurrentThread.ManagedThreadId == mainThreadId)
        ...
        {
            Debug.Log("OnSplashLoadSuccess");
        }

        public void OnSplashRenderSuccess(BUSplashAd ad)
        {
            Debug.Log("OnSplashRenderSuccess");
#if UNITY_ANDROID
            this.example.splashAd = ad;
#endif
            Debug.Log($"splash load OnRenderSuccess:  on main thread: {Thread.CurrentThread.ManagedThreadId == mainThreadId}");
            if(Thread.CurrentThread.ManagedThreadId == mainThreadId)
                this.example.information.text = "OnRenderSuccess";
#if UNITY_IOS
            Debug.Log("SetSplashInteractionListener Invoke");
        ...
        if (splashAdManager != null && this.activity != null)
        {
            ...
            {
                Debug.Log($"splash Ad OnAdDidShow  on main thread: {Thread.CurrentThread.ManagedThreadId == mainThreadId}");
                if(Thread.CurrentThread.ManagedThreadId == mainThreadId)
            ...
            {
                Debug.Log($"splash Ad OnAdWillShow  on main thread: {Thread.CurrentThread.ManagedThreadId == mainThreadId}");
                if(Thread.CurrentThread.ManagedThreadId == mainThreadId)
